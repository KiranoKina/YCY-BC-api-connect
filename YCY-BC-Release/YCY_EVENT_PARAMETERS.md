...
如果 `/ycy status` 显示 IM 发送成功、失败为 0，但 App 没有反应，可以执行：
...
| 编号 | 收件人 | ID 类型 | token |
| --- | --- | --- | --- |
| A | 自动目标 UID，例如 `16717` | 数字 ID | 带 |
| B | 自动目标 UID，例如 `16717` | 字符串 ID | 带 |
| C | `game_` 加目标 UID，例如 `game_16717` | 数字 ID | 带 |
| D | 自动目标 UID，例如 `16717` | 数字 ID | 不带 |

看 App 哪一次有反应，就能判断真正需要的收件人和包体格式。

如果 A/B/C/D 都没有反应，可以扫描数字指令 ID：
...
`probeall` 会把指定范围内的数字 ID 依次发送给当前目标用户。默认范围是 `0-20`，每个 ID 间隔约 600ms。
