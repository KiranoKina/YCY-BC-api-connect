...
(function () {
  ...
  function helpText() {
    ...
    return [
      ...
      "/ycy probeall [起始ID] [结束ID]：扫描数字指令 ID",
  ...
  async function runCommand(text) {
    ...
    if (sub === "probeall" || sub === "scan" || sub === "扫描") {
      const start = Number.isFinite(Number(argv[2])) ? Number(argv[2]) : 0;
      const end = Number.isFinite(Number(argv[3])) ? Number(argv[3]) : 20;
      const low = Math.max(0, Math.min(start, end));
      const high = Math.min(999, Math.max(start, end));
      const target = String(targetUser());
      const sent = [];
      for (let id = low; id <= high; id++) {
        try {
          await sendTo(target, id, undefined, { urgent: true });
          sent.push(id);
          await new Promise(resolve => setTimeout(resolve, 600));
        } catch (error) {
          local(`YCY：probeall 在 ID ${id} 失败：${error?.message || error}`, 15000);
          return true;
        }
      }
      local(`YCY：probeall 已发送 ${low}-${high} 到 ${target}。\n请看 App 哪个 ID 有反应：${sent.join(", ")}`, 30000);
      return true;
    }
