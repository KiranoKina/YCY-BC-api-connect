...
(function () {
  ...
  function commandObject(id, payload, options) {
    ...
    const hasControlPayload = Boolean(payload && (payload.waveform || payload.reset_strength));
  ...
  async function send(id, payload, options) {
    ...
    try {
      ...
      const message = instance.createTextMessage({
        ...
        payload: {
          text: JSON.stringify(commandObject(id, payload, options))
        }
      });
  ...
  async function runCommand(text) {
    ...
    if (sub === "map") {
      ...
      if (key === "all") {
        ...
        local(`YCY：已批量连接 hit=${hitId}，shock=${shockId}，touch=${touchId}，toy=${toyId}${waveform ? `，统一波形 ${waveform}` : ""}`);
        return true;
      }
      const id = argv[3] || "";
      if (!key || !id) {
    ...
    if (sub === "waveform") {
      ...
      updateSetting(s => {
        if (!s.waveforms) s.waveforms = {};
        const value = /^(off|none|default|默认)$/i.test(waveform) ? "" : waveform;
        if (key === "all") {
