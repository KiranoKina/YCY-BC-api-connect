...
(function () {
  ...
  function canSend(key, urgent) {
    ...
    if (!urgent && !current.consent) return "本次会话未授权";
    if (urgent) return "";
    const now = Date.now();
  ...
  function idForEventType(type) {
    if (type === "shock") return IDS.shock;
    if (type === "hit") return IDS.hit;
    return IDS.touch;
  }
  ...
  async function sendActivityLike(type, payload) {
    ...
    await send(idForEventType(type), {
      ...payload,
      intensity,
  ...
  function statusText() {
    ...
    return [
      "YCY 接入状态",
      `联动：${current.enabled ? "已启用" : "未启用"}；授权：${current.consent ? "已授权" : "未授权"}；调试：${current.debug ? "开" : "关"}`,
      `目标用户：${current.target || TARGET_USER}`,
      `connect_code：${parsed ? "已设置" : "未设置"}；IM：${chat ? "已登录" : "未登录"}；SDK：${existingTcSdk() ? "已加载" : sdkTask ? "加载中/已缓存" : "未加载"}`,
  ...
  async function runCommand(text) {
    ...
    if (sub === "test") {
      ...
      const ok = await send(id, {
        ...
        strength: type === "shock" ? 2 : 1,
        intensity,
        intensityPercent: Math.round(intensity * 100)
      });
      local(ok ? `YCY：已发送测试 ${type}` : `YCY：测试未发送，原因：${stats.lastSkip?.reason || "未知"}`);
      return true;
    }
  ...
  async function handleItemEquip(data) {
    ...
    if (data.Content === "ActionUse") {
      ...
      if (!itemName) return;
      await send(IDS.toySync, { source: "ActionUse", event: "itemAdded", assetName: itemName, assetGroupName: slot });
      ...
      await send(IDS.toySync, { source: "ActionRemove", event: "itemRemoved", assetName: itemName, assetGroupName: slot });
      ...
      itemStates.delete(slot);
      await send(IDS.toySync, { source: "ActionSwap", event: "itemSwapped", assetName: itemName, prevAssetName: prevItemName, assetGroupName: slot });
  ...
  async function syncToys(reason, force) {
    ...
    return send(IDS.toySync, { source: "Player.Appearance", reason, toys: data });
  }
  ...
  function hookToySync() {
    ...
    hookFunction("InventoryRemove", 3, (args, next) => {
      if (args?.[0]?.MemberNumber === playerNumber()) {
        ...
        if (name && slot) {
          itemStates.delete(slot);
          send(IDS.toySync, { source: "InventoryRemove", event: "itemRemoved", assetName: name, assetGroupName: slot })
            .catch(error => console.warn(MOD_NAME, error));
        }
      }
      return next(args);
    ...
    hookFunction("InventoryWear", 8, (args, next) => {
      ...
      if (args?.[0]?.MemberNumber === playerNumber()) {
        ...
        const slot = item?.Asset?.DynamicGroupName || item?.Asset?.Group?.Name;
        if (name && slot) {
          send(IDS.toySync, { source: "InventoryWear", event: "itemAdded", assetName: name, assetGroupName: slot })
