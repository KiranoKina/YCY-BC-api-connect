...
玩具和动作事件参考 XToys integration 的方式解析 BC 消息字段，不再只靠关键词。

强度计算参考 Razor Wings 的 DG-Lab 模块：先读取 `actionName + assetGroupName + assetName`，再通过动作、部位、道具修正规则计算 `intensity`。
...
- `TriggerShock0/1/2` 分别约为 `40/60/80`
- `Spank/Slap` 约为 `30`
- `Caress/Grope/Kiss` 约为 `10`
- 乳房、乳头、阴部、阴蒂等部位会提高强度
- 鞭子、拍子、棍类道具会提高强度

已接入：

- `Activity` 消息：读取 `FocusAssetGroup`、`ActivityName`、`ActivityAsset`、`TargetCharacter`、`SourceCharacter`
- `ActionUse` / `ActionRemove` / `ActionSwap`：同步物品穿脱和替换
- `PropertyShockPublishAction`：捕获电击发布
- `ExtendedItemSetOption` / `VibratorModePublish`：捕获手动调节震动/充气状态
- `InventoryWear` / `InventoryRemove`：捕获自己穿脱物品
- `FuturisticTrainingBelt` / `FuturisticChastityBelt`：捕获训练带震动和惩罚电击
- Portal Link Panties 的部分互动动作
