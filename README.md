...
即使在 `simple` 模式，如果设置了 `waveform` 或发送 `reset_strength`，脚本也会带上 payload，因为这些是 App 需要读取的控制参数。

`on`、`true`、`yes`、`开` 不会当成波形名发送，会被视为使用 App 默认波形。只有填写真实波形名称时才会发送 `payload.waveform`。
