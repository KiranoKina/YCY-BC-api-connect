...
| 模式 | 发送内容 | 适合场景 |
| --- | --- | --- |
| `simple` | 默认只发 `code`、`id`、`token` | App 只按指令 ID 触发效果 |
| `full` | 附带事件 payload | 调试、查看动作来源、需要 App 读取事件参数 |

即使在 `simple` 模式，如果手动设置了 `waveform` 或发送 `reset_strength`，脚本仍会带上 payload，因为这是 App 需要读取的控制参数。

`on`、`true`、`yes`、`开` 会被视为“使用 App 默认波形”，不会作为波形名发送。只有真实波形名称才会写入 `payload.waveform`。
